...
for disjunctive_cuts_type in [
    "linear", "linear2", "linear3",
]
    @suppress test_matrix_completion_disjunctivecuts(
...
for row_index in task_index:n_tasks:size(args_df, 1)
    ...
    disjunctive_cuts_type = convert(
        String,
        args_df[row_index, :disjunctive_cuts_type],
    )
